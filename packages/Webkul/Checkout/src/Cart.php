...
class Cart
{
	...
	public function saveAddresses(array $params): void
	{
		...
		// Set default shipping method to flatrate_flatrate if not set and cart has stockable items
		if ($this->cart && $this->cart->haveStockableItems() && ! $this->cart->shipping_method) {
			$this->saveShippingMethod('flatrate_flatrate');
		}
	}
	...
	public function collectTotals(): self
	{
		...
		/**
		 * Calculate gateway charge based on payment method configuration.
		 */
		...
		if ($this->cart->payment && $this->cart->payment->method) {
			try {
				...
				// If payment method not found or error, gateway charge remains 0
			}
		}
