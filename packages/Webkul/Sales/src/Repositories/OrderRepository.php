...
class OrderRepository extends Repository
{
	 ...
	 */
	public function createOrderIfNotThenRetry(array $data, int $attempt = 1, int $maxAttempts = 3)
	{
		$order = null;
		...
		try {
			...
			Log::error(
				'OrderRepository:createOrderIfNotThenRetry: Attempt ' . $attempt . ' failed: ' . $e->getMessage(),
				['data' => $data, 'attempt' => $attempt]
			);

			// Retry only for specific exceptions and within max attempts
			...
			// If max attempts reached or non-retryable exception, throw it
			throw $e;
		}
	...
	protected function shouldRetry(\Exception $e): bool
	{
		...
		// Retry only for deadlocks and lock timeouts, not for data validation errors
